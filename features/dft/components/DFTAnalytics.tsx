...
export default function DFTAnalytics() {
  const d = linePoints;
  const pathD = `M ${d[0].x} ${d[0].y} L ${d[1].x} ${d[1].y} L ${d[2].x} ${d[2].y} L ${d[3].x} ${d[3].y} L ${d[4].x} ${d[4].y} L ${d[5].x} ${d[5].y}`;
  const circles = d.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="4" fill="#C8A54B"></circle>`).join("");
    ...
    <div className="min-h-screen bg-surface-base text-text-primary">
      <div className="mx-auto max-w-[1440px]">
        <section id="view-analytics-dashboard" className="p-6 md:p-10">
          <div className="mb-6">
            <Link href="/dft/dashboard" className="mb-4 inline-flex items-center gap-2 rounded-lg border border-border-default bg-surface-overlay px-4 py-2 text-sm font-medium text-brand-gold transition hover:border-brand-gold hover:bg-surface-elevated">
              <ArrowLeft size={18} />
              Return back
            </Link>
            <h2 id="txt-analytics-title" className="text-xl font-bold text-text-primary">Team Performance Analytics</h2>
            <p id="txt-analytics-subtitle" className="mt-1 text-sm text-brand-gold">Scope: QA DIANA (DFT)</p>
          </div>
          ...
          <div className="analytics-filter-toolbar mb-6 flex flex-wrap items-center gap-4">
              ...
              <div className="analytics-filter-group flex items-center gap-2">
                ...
                <div className="timeframe-toggle-buttons flex items-center gap-1">
                  <button id="btn-tf-daily" className="btn btn-secondary btn-sm active border border-border-default bg-brand-gold/10 px-3 py-2 text-xs font-medium text-brand-gold">Daily</button>
                  <button id="btn-tf-weekly" className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold">Weekly</button>
                  <button id="btn-tf-monthly" className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold">Monthly</button>
                  <button id="btn-tf-quarterly" className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold">Quarterly</button>
                  <button id="btn-tf-yearly" className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold">Yearly</button>
                </div>
              </div>
              <div className="analytics-filter-group flex items-center gap-2">
                ...
                <div className="navigation-arrow-buttons flex items-center gap-2">
                  <button className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold"><ChevronLeft size={12} /> Prev</button>
                  <button id="btn-tf-current-label" className="btn btn-dark-accent btn-sm disabled border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-text-muted">AUG 15, 2026</button>
                   <button className="btn btn-secondary btn-sm border border-border-default bg-surface-overlay px-3 py-2 text-xs font-medium text-brand-gold">Next <ChevronRight size={12} /></button>
                </div>
              </div>
            </div>
            ...
            <div className="chart-card rounded-[16px] border border-border-default bg-surface-overlay p-5">
              <div className="chart-card-header mb-4 flex items-center justify-between">
                ...
                <div id="quarterly-average-container" className="analytics-summary-kpi hidden rounded-lg bg-surface-elevated px-3 py-1 text-right">
                  ...
                  <span id="linegraph-quarter-average" className="analytics-summary-value block text-sm font-bold text-brand-gold">94.50%</span>
                </div>
              </div>
              <div className="chart-container-surface">
                <svg id="svg-trend-line-chart" viewBox="0 0 600 240" className="native-svg-chart-surface h-full w-full">
                  ...
                  <path d={pathD} fill="none" stroke="#C8A54B" strokeWidth="3" />
                  <g>
                    {circles}
                    {labels}
                  </g>
