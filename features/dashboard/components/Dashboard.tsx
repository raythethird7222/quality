...
const accounts = [
  { label: "JS", description: "Job Scheduler", href: "/js", icon: Zap, borderClass: "border-brand-gold", fillClass: "bg-brand-gold", colorClass: "text-brand-gold", hoverClass: "hover:bg-brand-gold" },
  ...
  { label: "RM", description: "Resource Management", href: "/rm", icon: Users, borderClass: "border-brand-crimson", fillClass: "bg-brand-crimson", colorClass: "text-brand-crimson", hoverClass: "hover:bg-brand-crimson" },
...
export default function Dashboard() {
  ...
  return (
    <main className="min-h-screen bg-surface-base px-6 py-10 text-text-primary md:px-10">
      <div className="mx-auto max-w-[1440px]">
        <header className="flex items-start justify-between gap-6">
          <div>
            <h1 className="text-[32px] font-bold leading-none tracking-tight sm:text-[36px]">Welcome Back, Operator</h1>
            <p className="mt-2 text-[16px] leading-5 text-text-secondary">Select an operational managed tracking account system footprint to initialize dashboard views.</p>
          </div>
        </header>

        <section className="mt-5 rounded-2xl border border-brand-indigo border-t-4 bg-white px-6 py-5 shadow-sm" aria-label="Operator profile">
        ...
        <section className="mt-7" aria-labelledby="accounts-heading">
          ...
          <div className="mt-5 flex flex-wrap gap-7">
            {accounts.map((account) => (
              <Link key={account.label} href={account.href} aria-label={`Open ${account.label} dashboard`} className={`group relative flex h-[205px] w-[260px] flex-col items-center rounded-2xl border bg-white px-7 pt-4 transition hover:-translate-y-1 hover:text-white hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 ${account.borderClass} ${account.colorClass} ${account.hoverClass}`}>
